...
| network\_rulesets | One or more network rulesets. | `list(object({ defaul_action = string, ip_rules = list(string), subnet_ids = list(string), ignore_missing_virtual_network_service_endpoint = bool }))` | n/a | yes |
